...
class locate(ocr_character, screen_operate):
    def __init__(self):
        ...
        self.keyword = ["总力战信息", "排名", "奖励信息","调整编队","部队","选择值日员","更新","增益","袭击","火车","确认","扫荡完成",
                        "任务信息","材料","进入剧情","剧情信息","选择剧情","变更","全部收纳","邀请券","库存","预设","收益","家具信息",
                        "基本信息",
                        "说明","主要能力值","游戏","图像","音量","制造","种类","使用","单价","成员等级","成员列表","筛选","排序",
                        "道具","装备","主线","支线","档案","普通","困难","帮助","邀请券","礼物","特别委托","据点","工厂","广场",
                        "信用","讲堂","高架","铁路","预设","点击继续","公告","活动","通知","跳过","预告","咖啡厅","日程","成员",
                        "工作任务","编队","小组","制造工坊","商店","招募","业务区","任务","故事","悬赏通缉","帮助","选项","菜单",
                        "青辉石","礼包","购买","账号信息","账号设置"]
    ...
    def return_location(self):
        t1 = time.time()
        self.return_keyword_appear_time(self.img_ocr(t.get_screen_shot_path()))
        t2 = time.time()
        print(t2-t1)
        if self.pd(["选项","游戏","图像","音量"], [1,1,1,1]):
            ...
            if self.pd(["公告", "活动","预告"], [1, 1, 1]):
                return "main_page_notice"
            elif self.pd(["青辉石"], [2]) or self.pd(["礼包"], [2]):
                return "main_page_recharge"
            elif self.pd(["菜单"], [1]):
                return "main_page_menu"
            elif self.pd(["单价"],[1]):
                return "main_page_power"
            elif self.pd(["账号设置"],[1]):
                return "main_page_account_message"
            else:
                return "main_page"
    ...
    def return_keyword_appear_time(self,string):
